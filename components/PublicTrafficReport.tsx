...
export const PublicTrafficReport: React.FC<PublicTrafficReportProps> = ({ publicId, embedded = false, reportDataOverride = null, titleOverride = null, createdAtOverride = null }) => {
  ...
  const [topAdsVisible, setTopAdsVisible] = useState(4);
  const [topCampaignGroupsVisible, setTopCampaignGroupsVisible] = useState(4);
  ...
  const hasThumb = (item: { thumbnailUrl?: string }) => Boolean(item.thumbnailUrl && !item.thumbnailUrl.startsWith('data:') && item.thumbnailUrl.startsWith('http'));
  ...
  const visibleGroupedTopAds = groupedTopAds.slice(0, topCampaignGroupsVisible);
  const hasMoreTopAds = groupedTopAds.some(([, ads]) => ads.length - 1 > topAdsVisible);
  const hasMoreTopCampaignGroups = groupedTopAds.length > topCampaignGroupsVisible;
  ...
  const investmentDistributionRows = [...campaigns]
    .filter((camp) => camp.spend > 0)
    .sort((a, b) => b.spend - a.spend || b.results - a.results || (b.idc ?? 0) - (a.idc ?? 0))
    .slice(0, 24);
      ...
      <div style={{ maxWidth: embedded ? '100%' : 900, margin: '0 auto', padding: embedded ? '0' : '0 24px 60px' }}>
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <div className="no-print" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <select value={topAdsFilter} onChange={(e) => { setTopAdsFilter(e.target.value); setTopAdsVisible(4); setTopCampaignGroupsVisible(4); }} style={selectStyle}>
                  <option value="all">Todos os resultados</option>
                  {resultTypeOptions.map((label) => <option key={label} value={label}>{label}</option>)}
                </select>
              </div>
              <div style={{ fontSize: 11, color: '#8a95a8', fontFamily: 'DM Mono, monospace' }}>Exibindo {visibleGroupedTopAds.length} de {groupedTopAds.length} campanhas</div>
            </div>
            {visibleGroupedTopAds.map(([campaignName, ads]) => {
                ...
                const runners = rankedAds.slice(1, topAdsVisible + 1);
                ...
                return (
                  <div key={campaignName} style={{ marginBottom: 28 }}>
                    ...
                    <div style={{ background: 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 14, padding: '18px 20px', marginBottom: 8, display: 'flex', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                      <a href={champion.id ? `https://www.facebook.com/ads/library/?id=${champion.id}` : undefined} target="_blank" rel="noopener noreferrer" title="Ver anúncio na Biblioteca de Anúncios" style={{ position: 'relative', width: 80, height: 80, background: '#141921', borderRadius: 10, border: '2px solid rgba(245,158,11,0.3)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, textDecoration: 'none', color: 'inherit' }}>{hasThumb(champion) ? <img src={champion.thumbnailUrl} alt="" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /> : 'AD'}</a>
                      <div style={{ flex: 1, minWidth: 180 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                          ...
                          {champion.id && <a href={`https://www.facebook.com/ads/library/?id=${champion.id}`} target="_blank" rel="noopener noreferrer" title="Ver anúncio na Biblioteca de Anúncios" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 6, background: '#141921', border: '1px solid #253040', color: '#8a95a8', textDecoration: 'none', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>Abrir <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15,3 21,3 21,9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a>}
                        </div>
                    ...
                    {runners.length > 0 && <div style={{ display: 'grid', gridTemplateColumns: `repeat(${runners.length}, 1fr)`, gap: 8 }}>{runners.map((ad, index) => { const color = IDC_COLORS[ad.idcClass] ?? '#8a95a8'; return <div key={ad.id} style={{ background: '#0d1117', border: '1px solid #1e2733', borderRadius: 12, padding: '14px 16px', display: 'flex', gap: 12, alignItems: 'flex-start' }}><a href={ad.id ? `https://www.facebook.com/ads/library/?id=${ad.id}` : undefined} target="_blank" rel="noopener noreferrer" title="Ver anúncio na Biblioteca de Anúncios" style={{ width: 52, height: 52, background: '#141921', borderRadius: 8, border: '1px solid #1e2733', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, textDecoration: 'none', color: 'inherit' }}>{hasThumb(ad) ? <img src={ad.thumbnailUrl} alt="" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /> : 'AD'}</a><div style={{ flex: 1, minWidth: 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}><span style={{ width: 18, height: 18, borderRadius: 5, background: '#141921', border: '1px solid #253040', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#8a95a8', flexShrink: 0 }}>{index + 2}</span><span style={{ fontSize: 9, fontWeight: 700, color, fontFamily: 'DM Mono, monospace' }}>IDC {ad.idc}</span>{ad.id && <a href={`https://www.facebook.com/ads/library/?id=${ad.id}`} target="_blank" rel="noopener noreferrer" title="Ver anúncio na Biblioteca de Anúncios" style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 7px', borderRadius: 5, background: '#141921', border: '1px solid #253040', color: '#8a95a8', marginLeft: 'auto', textDecoration: 'none', fontSize: 10, fontWeight: 600, flexShrink: 0 }}>Abrir <svg xmlns="http://www.w3.org/2000/svg" width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15,3 21,3 21,9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a>}</div><div style={{ fontWeight: 600, color: '#c5d0de', fontSize: 12, marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ad.name}</div><div style={{ fontSize: 11, color: '#8a95a8' }}>{brl(ad.spend)} &bull; <span style={{ color: '#22c55e' }}>{ad.results} {ad.resultLabel}</span></div><div style={{ fontSize: 10, color: '#4d5a6e', marginTop: 3 }}>CTR {pct(ad.ctr, 1)} &bull; CPC {brl(ad.cpc)}{ad.hookRate > 0 ? ` - Hook ${pct(ad.hookRate * 100, 1)}` : ''}</div></div></div>; })}</div>}
                  </div>
                );
              })}
            ...
            {(hasMoreTopCampaignGroups || hasMoreTopAds || topAdsVisible > 4 || topCampaignGroupsVisible > 4) && (
        ...
        {investmentDistributionRows.length > 1 && (
          <div style={{ marginBottom: 40 }}>
            ...
            <div style={{ marginTop: 8, fontSize: 11, color: '#8a95a8' }}>Exibindo os 24 criativos com maior investimento no período.</div>
          </div>
        )}
