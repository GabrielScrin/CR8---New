...
as $$
  ...
  where id = p_source_id and secret_hash = digest(convert_to(p_token, 'UTF8'), 'sha256')
  limit 1;
$$;
...
drop policy if exists "Managers can read inbound sources" on public.integration_inbound_sources;
...
drop policy if exists "Managers can read outbound endpoints" on public.integration_outbound_endpoints;
...
drop policy if exists "Service role can insert webhook events in" on public.webhook_events_in;
create policy "Service role can insert webhook events in" on public.webhook_events_in for insert with check (false);
drop policy if exists "Admins can read webhook events in" on public.webhook_events_in;
